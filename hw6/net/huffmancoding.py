            ...
            if 'conv' in name:
                ...
                for i in range(shape[0]):
                    for j in range(shape[1]):
                        ...
                        mat_lenth.append(np.int32(len(mat.data)))
                        mat_lenth.append(np.int32(len(mat.indices)))
                        mat_lenth.append(np.int32(len(calc_index_diff(mat.indptr))))
                ...
                t3, d3 = huffman_encode(np.array(mat_lenth), name+f'_{form}_lenth', directory)
    ...
    for name, param in model.named_parameters():
        if 'weight' in name:
            ...
            if 'conv' in name:
                ...
                lenth = huffman_decode(directory, name+f'_{form}_lenth', dtype='int32')

                data_start, data_end = 0
                indices_start, indices_end = 0
                indptr_start, indptr_end = 0

                idx = 0
                for i in range(shape[0]):
                    for j in range(shape[1]):

                        data_end = lenth[idx * 3] + data_start
                        mat_data = data[data_start : data_end]
                        data_start = data_end

                        indices_end = lenth[idx * 3 + 1] + indices_start
                        indices_data = indices[indices_start : indices_end]
                        indices_start = indices_end

                        indptr_end = lenth[idx * 3 + 2] + indptr_start
                        indptr_data = reconstruct_indptr(indptr[indptr_start : indptr_end])
                        indptr_start = indptr_end

                        # Construct matrix
                        mat = matrix((mat_data, indices_data, indptr_data), shape[2 : 4])
